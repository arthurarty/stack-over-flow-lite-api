...
class Database:
    ...
    def query_all(self, table_name):
        ...
        self.cursor.execute(
            "SELECT row_to_json(row) FROM (SELECT * FROM %s) row;" % (table_name))
    ...
    def delete_any(self, table_name, primary_column, item_id):
        ...
        delete_command = "DELETE FROM %s WHERE %s = %s" % (
            table_name, primary_column, item_id)
    ...
    def delete_question(self, question_id):
        ...
        delete_command = "DELETE FROM questions WHERE question_id = %s" % (
            question_id)
        self.cursor.execute(delete_command)

    def return_user_id(self, table_name, where_column, row_id):
        user_id_command = "SELECT user_id FROM %s WHERE %s = %s" % (
            table_name, where_column, row_id)
