...
def add_answer_to_question(question_id):
    ...
    if title:
        ...
        new_answer = Answer(
            question_id, request.json.get('title'), current_user)
...
def mark_answer_preferred(question_id, answer_id):
    ...
    if current_user in output:
        value = "True"
        db_conn.update_record("answers", "preferred",
...
def edit_answer(question_id, answer_id):
    ...
    if title:
        ...
        current_user = get_jwt_identity()
        if current_user in output:
            db_conn.update_record("answers", "title", request.json.get(
                'title'), "answer_id", answer_id)
